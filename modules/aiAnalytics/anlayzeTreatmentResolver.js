...
exports.analyzeTreatmentsResolver = async ({
  ...
  try {
    const timeBasedAnalysis =
      ...
      (treatmentName && (startDate || endDate));

    if (timeBasedAnalysis && (!startDate || !endDate)) {
      return {
        success: false,
        error:
          "Start date and end date are required for time-based analysis. Please specify the time period.",
        analysisType,
        suggestion:
